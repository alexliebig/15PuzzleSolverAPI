...
extern "C" {
    ...
    int release_memory(int* pArray)
    {
        delete[] pArray;
        return 0;
    }

    bool is_valid_puzzle(int* pArray, int boardSize)
    {
    ...
    int* solve_puzzle(int* pArray, int boardSize, int& solvedPuzzleSize)
    {
        std::vector<std::vector<int>> puzzle;
        std::vector<int> row(boardSize);
        for (int i = 0; i < boardSize; i++)
